...
class Creature():
    ...
    def add_spell_feature(self, title: str, section: Section):
        text = [l.text for l in section.lines]
        ...
        ability_re = re.compile("spellcasting ability is ({})".format("|".join(constants.enum_values(constants.ABILITIES))), re.IGNORECASE)
    ...
    def add_legendary_block(self, section: Section):
        self.data["legendary_block"] = section.get_section_text().strip()
    ...
    def add_feature(self, section : Section):
        ...
        if "spellcasting" in title.lower():
            self.add_spell_feature(title, section)
        else:
            self.add_normal_feature(title, text)
